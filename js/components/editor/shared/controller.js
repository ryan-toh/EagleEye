...
function initResizableEditorColumns() {
  ...
  explorer.addEventListener('pointermove', event => {
    ...
    const panes = [...explorer.querySelectorAll(':scope > .decision-explorer__pane')];
    const isResizable = pane && panes.indexOf(pane) < panes.length - 1;
    const nearRightEdge = isResizable && pane.getBoundingClientRect().right - event.clientX < 10;
  ...
  explorer.addEventListener('pointerdown', event => {
    ...
    if (index < 0 || index === panes.length - 1 || pane.getBoundingClientRect().right - event.clientX >= 10) return;
  ...
  explorer.addEventListener('pointermove', event => {
    ...
    const nextWidth = Math.max(
      minWidths[index],
      Math.min(widths[index] + event.clientX - startX, combinedWidth - minWidths[index + 1])
    );
    const adjustedWidths = [...widths];
    adjustedWidths[index] = nextWidth;
    adjustedWidths[index + 1] = combinedWidth - nextWidth;
    explorer.style.gridTemplateColumns = adjustedWidths.map(width => `${width}px`).join(' ');
  });
  ...
  const stopResizing = event => {
    ...
    if (explorer.hasPointerCapture(event.pointerId)) explorer.releasePointerCapture(event.pointerId);
  };
